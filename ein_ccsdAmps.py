...
#              F_ae[a,e]-= (1/2)*tau_tilde[a,f,m,n]*IJAB[m,n,e,f] #MO[m,n,e+O,f+O]
    V = NB - O
    F_ae = np.zeros((V, V))
    F_ae += (1 - np.eye(V)) * Fock[O:, O:]
    F_ae -= 0.5 * np.einsum('me,am->ae', Fock[:O, O:], t1)
    F_ae += np.einsum('fm,mafe->ae', t1, MO[:O, O:, O:, O:])
    F_ae -= 0.5 * np.einsum('afmn,mnef->ae', tau_tilde, MO[:O, :O, O:, O:])
    #Equation (5) error
    F_mi=np.zeros((O, O))
 #   for m in range(O):
 #     for i in range(O):
 ...
 #             F_mi[m,i]+=(1/2)*tau_tilde[e,f,i,n]*IJAB[m,n,e,f]
    ...
    F_mi += np.einsum('en,mnie->mi', t1, IJKA)
...
#            F_me[m,e]+=t1[f,n]*IJAB[m,n,e,f] #MO[m,n,e+O,f+O]
    F_me = np.zeros((O, V))
    F_me += Fock[:O, O:]
    F_me += np.einsum('fn,mnef->me', t1, MO[:O, :O, O:, O:])
...
#                W_mnij[m,n,i,j]+=(1/4)*tau[e,f,i,j]*IJAB[m,n,e,f] #MO[m,n,e+O,f+O]
    W_mnij = np.zeros((O, O, O, O))
    W_mnij += MO[:O, :O, :O, :O]
    W_mnij += np.einsum('ej,mnie->mnij', t1, MO[:O, :O, :O, O:])
    W_mnij -= np.einsum('ei,mnje->mnij', t1, MO[:O, :O, :O, O:])
    W_mnij += 0.25 * np.einsum('efij,mnef->mnij', tau, MO[:O, :O, O:, O:])
...
#                W_abef[a,b,e,f]+=(1/4)*tau[a,b,m,n]*IJAB[m,n,e,f] #MO[m,n,e+O,f+O]
    W_abef = np.zeros((V, V, V, V))
    W_abef += MO[O:, O:, O:, O:]
    W_abef -= np.einsum('bm,amef->abef', t1, MO[O:, :O, O:, O:])
    W_abef += np.einsum('am,bmef->abef', t1, MO[O:, :O, O:, O:])
    W_abef += 0.25 * np.einsum('abmn,mnef->abef', tau, MO[:O, :O, O:, O:])
...
#                W_mbej[m,b,e,j]-= ((1/2)*t2[f,b,j,n]+t1[f,j]*t1[b,n])*IJAB[m,n,e,f] #MO[m,n,e+O,f+O]
    W_mbej = np.zeros((O, V, V, O))
    W_mbej += MO[:O, O:, O:, :O]
    W_mbej += np.einsum('fj,mbef->mbej', t1, MO[:O, O:, O:, O:])
    W_mbej -= np.einsum('bn,mnej->mbej', t1, MO[:O, :O, O:, :O])
    W_mbej -= 0.5 * np.einsum('fbjn,mnef->mbej', t2, MO[:O, :O, O:, O:])
    W_mbej -= np.einsum('fj,bn,mnef->mbej', t1, t1, MO[:O, :O, O:, O:])
...
#        t1_f[a,i]=t1_f[a,i]/D1[a,i]
    ...
    t1_f -= 0.5 * np.einsum('efim,maef->ai', t2, MO[:O, O:, O:, O:])
    t1_f -= 0.5 * np.einsum('aemn,nmei->ai', t2, MO[:O, :O, O:, :O])
    t1_f -= np.einsum('fn,naif->ai', t1, MO[:O, O:, :O, O:])
    t1_f /= D1
  return t1_f
