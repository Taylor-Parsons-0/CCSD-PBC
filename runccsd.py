...
for i in range(O):
  for j in range(O):
    for a in range(V):
      for b in range(V):
        t2[a,b,i,j]+=IJAB[i,j,a,b]/(Fock[i,i]+Fock[j,j]-Fock[a+O,a+O]-Fock[b+O,b+O])
...
tau=tauEq(O, V, NB, Fock, t1, t2, MO, IJKL, ABCD, IABC, IJAB, IJKA, AIBC, IABJ, IJAK, IAJB, ABCI, IAJK, 1)
...
while DiffE>0.000000000000001 or DiffT1>0.00000000001 or DiffT2>0.00000000001 or t1RMSE>0.00000000001 or t2RMSE>0.00000000001:
  E_Corr1=E_Corr2
  tau_tilde=tau_tildeEq(O, V, NB, Fock, t1, t2, MO, IJKL, ABCD, IABC, IJAB, IJKA, AIBC, IABJ, IJAK, IAJB, ABCI, IAJK, 1)
  tau=tauEq(O, V, NB, Fock, t1, t2, MO, IJKL, ABCD, IABC, IJAB, IJKA, AIBC, IABJ, IJAK, IAJB, ABCI, IAJK, 1)
...
#    writer.write(f"Time for Step {N}: {time.time()-start}\n")
  ...
  t1=np.copy(t1_f)
  t2=np.copy(t2_f)
  E_Corr2=CCSD(O, V, NB, Fock, t1, t2, MO, IJKL, ABCD, IABC, IJAB, IJKA, AIBC, IABJ, IJAK, IAJB, ABCI, IAJK, 1, tau)
